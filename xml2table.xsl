  ...
  <html>
    ...
    <body>
      <table border="1">
        ...
        <xsl:for-each select="//Livre"><xsl:call-template name="livre"/></xsl:for-each>
      </table>
    </body>
  </html>
  </xsl:template>
...
<td><xsl:value-of select="Ex[1]/Cote"/> (<xsl:value-of select="count(Ex)"/>)</td>
<td>
  <xsl:for-each select="aleph">
    ...
    <xsl:choose>
     ...
     (<xsl:value-of select="nb_items"/>) - <xsl:for-each select="loc"><xsl:value-of select="."/></xsl:for-each> -- // Dernier retour : <xsl:for-each select=".//last_return"><xsl:if test="position() != 1">, </xsl:if><xsl:value-of select="."/></xsl:for-each>
  </xsl:for-each>
</td>
<td>
